...
const getHealthCounters = `-- name: GetHealthCounters :one
SELECT
...
type GetHealthCountersRow struct {
...
func (q *Queries) GetHealthCounters(ctx context.Context) (GetHealthCountersRow, error) {
	row := q.db.QueryRow(ctx, getHealthCounters)
	var i GetHealthCountersRow
	err := row.Scan(&i.FailedVoices, &i.UsersNeedRelogin, &i.ChannelsWithError)
	return i, err
}
